...
RMSE_mod = {'S-DNN': [], 'T-DNN': [], 'Causal Forest': [], 'R-DNN': [],
...
for b in range(B):
    ...
    # Estimate p(A | X)
    mod_A = LogisticRegression(solver='lbfgs', max_iter=1000)
    mod_A.fit(X=X_train, y=A_train)

    pred_A = mod_A.predict_proba(X_test)[:, 1]
    ...
    pred_1 = mod_S.predict(X=np.c_[X_test, np.ones(test_size)])
    pred_0 = mod_S.predict(X=np.c_[X_test, np.zeros(test_size)])

    RMSE_mod['S-DNN'].append(RMSE(ITE_test, pred_1 - pred_0))
    ...
    pred_1 = mod_dict['mod_T1'].predict(X=X_test)
    pred_0 = mod_dict['mod_T0'].predict(X=X_test)

    RMSE_mod['T-DNN'].append(RMSE(ITE_test, pred_1 - pred_0))
    ...
    pred_CF = causal_forest.effect(X=X_test)

    RMSE_mod['Causal Forest'].append(RMSE(ITE_test, pred_CF))
    ...
    with torch.no_grad():
        ...
        mod_RDNN.eval()
        pred_ITE_A_RDNN = mod_RDNN(X_test)[:, 1].reshape(-1)

    RMSE_mod['R-DNN'].append(RMSE(ITE_test, pred_ITE_A_RDNN))
    ...
    with torch.no_grad():
        ...
        mod_sep_RDNN.eval()
        _, pred_ITE_A_sep_RDNN = mod_sep_RDNN(X_test)

    RMSE_mod['R-sep-DNN'].append(RMSE(ITE_test, pred_ITE_A_sep_RDNN))
    ...
    with torch.no_grad():
        ...
        mod_RNAM.eval()
        pred_ITE_A_RNAM = mod_RNAM(X_test)[:, 1].reshape(-1)

    RMSE_mod['R-NAM'].append(RMSE(ITE_test, pred_ITE_A_RNAM))
    ...
    with torch.no_grad():
        ...
        mod_sep_RNAM.eval()
        _, pred_ITE_A_sep_RNAM = mod_sep_RNAM(X_test)

    RMSE_mod['R-sep-NAM'].append(RMSE(ITE_test, pred_ITE_A_sep_RNAM))
    ...
    with torch.no_grad():
        ...
        mod_Rmix.eval()
        _, pred_ITE_A_Rmix = mod_Rmix(X_test)

    RMSE_mod['R-sep-mix'].append(RMSE(ITE_test, pred_ITE_A_Rmix))



print('\n\nCATE:\n')
for i, j in RMSE_mod.items():
